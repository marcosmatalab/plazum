...
> **La revisión hostil del 25-08-2026 encontró 7 hallazgos, uno bloqueante. Arreglados los tres bloques y reabiertas las casillas con el código delante.** El bloqueante era de clase, no de caso borde: todo lo que el receptor debe aportar estaba guardado como campo que escribe el emisor, así que la verificación comparaba al emisor consigo mismo. Arreglo: `ContextoReceptor` y `ledger.Confianza` entran por parámetro, los campos del fichero pasan a `AnclasDeclaradas` y `ClavesDeclaradas` y solo se contrastan, el `Checkpoint` lleva `Token` que se verifica de verdad, y el `Expediente` pasa a `CadenaV2`. Lo vigila `TestLaConfianzaNoViajaEnElFichero` (AST, con su control negativo). Los ataques viven en `hostil_test.go` de cada paquete, ya en verde como regresión. Barrido de mutación: de 16 comprobaciones sin control negativo aislado a 1, y esa queda anotada en el código como estructuralmente redundante.
...
- [ ] Sello real en el expediente demo. Hoy el demo lleva un token de relleno, así que `dutiq verify demo` con raíces de TSA de verdad no puede dar verde: avisa y se niega, que es lo correcto pero es una mala primera experiencia. Hace falta generar el demo con un sello RFC 3161 auténtico (o con una CA de demostración cuyo certificado viaje en el contexto de ejemplo) y publicar un `contexto-demo.json` al lado. La verificación de sellos de verdad ya tiene su suite completa en adaptadores/tsa con TSA falsa
