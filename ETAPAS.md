...
- [~] serve con html/template + htmx vendorizado, go:embed, sesiones. **La superficie está y las dos mitades encajan** (`cableado_test.go` monta las pantallas reales dentro del servidor real y comprueba que las rutas entran en la puerta que enumera, que ninguna del conjunto muta y que las cabeceras de seguridad llegan también a las páginas de la aplicación). **Lo que falta es la orden `dutiq serve`**, o sea que hoy un comprador no puede arrancar esto: la superficie existe y no hay forma de levantarla. Depende del adaptador de catálogo, que está en construcción; sin él la interfaz saldría con las claves en crudo
