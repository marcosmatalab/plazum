...
> **Puertos congelados el 25-08-2026, antes de implementar nada.** `puertos/etapa2.go` define las interfaces contra las que compilan todos los frentes, `congelacion_test.go` las congela leyendo el AST (cambiar una firma pone el test en rojo con instrucciones de parar y preguntar) y `puertos/contrato/` son las suites de comportamiento que toda implementación tiene que pasar. Congelar la firma no congela el significado: una `Sesion` que devuelve nil en `ComprobarCSRF` satisface la interfaz y no protege de nada, por eso existen las suites. Verificado por mutación que tienen dientes.

- [x] Puertos de la etapa 2 definidos y congelados, con godoc y suites de contrato. Cero implementación. Ocho interfaces: Servidor, Sesion (con el CSRF dentro, porque un CSRF que no está atado a su sesión no es CSRF), Plantilla, UIGenerada, Catalogo (i18n), Actualizador, Diagnostico y Seguridad. Las dos últimas de la lista pedida se llamaban i18n y diagnóstico; UIGenerada y Seguridad son AÑADIDOS sobre lo pedido, porque los frentes (b) y (a) no tendrían contra qué compilar sin ellas
