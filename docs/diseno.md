# obligo: el GRC de continuidad. Diseño definitivo
...
1. **Toda pantalla responde "por qué veo esto"**: derivación completa a un clic (regla, paquete, cita). Ya construido en el motor (`obligo explain`).
2. **Todo error es accionable**: causa, arreglo y runbook embebido en el binario (`/ayuda` offline, es/en/de). Nada depende de una web.
3. **`obligo doctor`**: diagnóstico local completo con salida copiable a un issue, datos personales redactados. El bug report lo genera la herramienta.
4. **`obligo demo`**: empresa sintética completa con relojes corriendo. El evaluador ve el producto lleno en dos minutos sin conectar nada.
5. **`obligo update`**: backup previo, migración, rollback de un comando. La causa número uno de tickets de self-hosted, cerrada por diseño.
...
|---|---|---|
| **obligo Cloud** (gestionado UE, la línea ancla) | 290 €/mes (≤100 empl.) · 590 (≤300) · 990 (≤1.000) · 1.690 (>1.000, Postgres+HA) | instancia operada, corpus con plazo objetivo, sello eIDAS de cada actualización, soporte con SLA, carpeta de compras |
...
| Eje | CISO Assistant | Eramba | Comp AI / Probo / Openlane | Hyperproof | ServiceNow IRM | Vanta / Drata | Cyberday | **obligo** |
|---|---|---|---|---|---|---|---|---|
...
| 9 | Checkout + licencia firmada + obligo Cloud con lista de espera | D14 | 3-4 |
