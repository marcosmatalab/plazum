...
- **`docs/instantanea.md` lleva los pesos antiguos** y no se toca: es una foto fechada cuyos números envejecen a la vez que el árbol. Se vuelve a hacer entera o no se hace; retocarle una celda la convertiría en una foto que finge estar viva. Las notas «Hoy» de arriba salen de ella y **arrastran su fecha**.
- **Y esta misma frase llevaba dentro un ejemplo caducado**, que es la prueba de que el problema no era la instantánea sino citarla: daba dos recuentos de casos de test, el de la foto y el «de hoy», y para cuando alguien la leyó ninguno de los dos era cierto. **La corrección había caducado igual que lo que corregía**, y por eso aquí ya no hay ninguna cifra: reproducir el número para denunciarlo es volver a ponerlo en circulación. Desde el 04-09-2026 lo impide una puerta (`instantanea_congelada_test.go`), que **rechazó esta misma frase** cuando su primer borrador citaba el número mientras explicaba que no se cita. Ningún número que sólo viva en la instantánea aparece en otro documento del repositorio: una foto vieja es honesta si nadie la usa como dato vivo.
