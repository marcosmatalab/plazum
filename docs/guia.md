# obligo: la guía definitiva de construcción
...
- El propio cumplimiento CRA de obligo (SBOM, CVD, ventana de soporte declarada) arranca aquí, no en la etapa 8.

**La marca, antes de invertir en ella:** búsqueda de anterioridades y solicitud EUIPO (clases 9 y 42, ~850 €) en la semana 0. Aviso encontrado por la revisión: existe una fintech internacional llamada Obligo, y "Obligo" es término financiero común en alemán, justo el mercado del año 2. **Decisión de nombre definitivo antes del primer release público**, con alternativa preparada si la búsqueda pinta mal.
...
Toda evidencia con fichero (PDF del acta, captura, informe del auditor) se guarda **content-addressed dentro de SQLite**: tabla `blobs(hash, datos_cifrados, tamano)`, cifrada por entrada con el mismo régimen del ledger (AEAD con compromiso de clave), chunking a partir de 32 MB. Por qué dentro y no en disco: **un solo fichero que respaldar**: Litestream replica la base y con ella viaja todo (evidencias incluidas), `obligo update` hace backup de una cosa, el restore drill restaura una cosa, y el runbook del Cloud no tiene una segunda ruta de backup que olvidar. El coste (base más gorda) se acepta y se mide: presupuesto en CI de tamaño de base con la demo.
...
**4.6. La demo alojada, definida.** Además del `obligo demo` local: una instancia pública efímera con reset horario, sin LLM expuesto y sin registro, con presupuesto (~10 €/mes). Es el "pruébalo lleno en dos minutos" del que depende el self-serve.
...
4. comunicación de brechas como encargado (el art. 33 aplica a obligo)
...
5. **Certificarse a sí mismo usando obligo** (año 2): el Cloud con su ENS o ISO 27001, con el expediente público. Es a la vez la prueba del producto, el contenido de marketing definitivo y lo que desbloquea al comprador alemán en 2028.
