...
>
> **EL «78 de 144» DE AQUI Y EL «78 sobre 144» DEL README NO SON LA MISMA CIFRA, y que hoy coincidan las DOS mitades es CASUALIDAD.** Esta cuenta CASILLAS DEL PLAN, derivadas del arbol de este fichero por `estado_del_plan_test.go`. La del README cuenta RELOJES DEL CORPUS sobre PUNTOS CENSADOS, y su denominador sale de sumar las siete filas con denominador de `paquetes/marcos-v1.json`. No tienen ninguna relacion: ni el numerador ni el denominador miden lo mismo. **El dia que una se mueva, la reaccion natural de quien lea los dos documentos sera cuadrarlas, y cuadrarlas corrompe una en silencio.** Lo vigila `TestLasDosCifrasQueCoincidenPorCasualidadLoDicen`.
>
