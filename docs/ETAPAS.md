...
>
> **79 de 146 casillas** cerradas, y **266 relojes escritos** en el corpus.
>
> **EL «79 de 146» DE AQUI Y EL «78 sobre 144» DE `docs/cobertura-v1.md` NO SON LA MISMA CIFRA, y que se parezcan tanto no significa nada.** Hasta el 22-09-2026 coincidian ENTERAS, y al marcar dos casillas se separaron en uno: **eso las hace mas confundibles, no menos**, porque una diferencia de uno se lee como un descuadre que alguien deberia arreglar. Esta cuenta CASILLAS DEL PLAN, derivadas del arbol de este fichero por `estado_del_plan_test.go`. La de `docs/cobertura-v1.md` cuenta RELOJES DEL CORPUS sobre PUNTOS CENSADOS, y su denominador sale de sumar las siete filas con denominador de `paquetes/marcos-v1.json`. No tienen ninguna relacion: ni el numerador ni el denominador miden lo mismo. **El dia que una se mueva, la reaccion natural de quien lea los dos documentos sera cuadrarlas, y cuadrarlas corrompe una en silencio.** Lo vigila `TestLasDosCifrasQueCoincidenPorCasualidadLoDicen`.
>
...
>
> **LA DERIVA, que las dos cifras de arriba no ensenan.** Instantanea anterior: **31 de 100, el 25-08-2026**, en `a62b8d0`. Esta es del **22-09-2026**, o sea **28 dias** despues: **+48 cerradas** y **+46 abiertas**, **1,7 al dia contra 1,6**, y las pendientes de **69 a 67**. El plan crece un poco mas deprisa de lo que se cierra, y **eso no es malo por si solo**: D-22 abrio siete casillas porque destapo trabajo que ya estaba dentro de otras dos, y el trabajo estaba igual antes de contarlo. Lo que si era malo es que no se viera, porque con dos cifras que solo suben **un re-corte que ensancha el plan se lee igual que uno que lo estrecha**. El ancla es un hecho del pasado y no caduca; los deltas, las tasas y las pendientes los deriva `estado_del_plan_test.go` de ella y del arbol. **Y LA TASA BAJO A LA MITAD SIN QUE EL RITMO CAMBIARA, asi que se dice antes de que alguien lo lea mal**: el 22-09-2026 la ventana paso de 16 dias a 28 y las casillas apenas se movieron (78 a 79), o sea de 2,9 al dia a 1,7. No es que el proyecto se frenara: es que esos doce dias fueron a documentacion, release y puertas, y **el contador de casillas no mide ese trabajo**, igual que no mide el de corpus. Es la misma razon por la que el estado va con dos numeros y ninguno de los dos solo dice la verdad.
>
...
>
> **BARRIDO DE LAS 58 CERRADAS (04-09-2026): 58 verificadas ciertas, 0 desmarcadas, 1 que no se puede comprobar por ejecución.** La evidencia de cada una, en [`docs/hallazgos/barrido.md`](https://github.com/marcosmatalab/plazum/blob/a1ef407850fba1887977cc255e7eddfd9173d962/docs/hallazgos/barrido.md). **54 de las 58 se verificaron con una puerta, con el binario ejecutado o contra la API de GitHub**; las otras cuatro descansan sólo en que un fichero está donde dice. La que no se puede comprobar es el **workflow de release**: está escrito y **no se ha ejecutado nunca**, ni una vez, y el remoto no tiene ni una etiqueta.
>
...
- [x] DECISIÓN DE MARCA, tomada e implantada el 26-08-2026. **El producto se llamaba DUTIQ y ahora se llama PLAZUM.** Lo que mató a DUTIQ: TMview, 25-08-2026, DOS EUTM de "Utiq" REGISTRADAS (no solicitadas), del titular Utiq SA/NV, **018838934** denominativa y **018838908** figurativa, vigentes hasta el 21-02-2033, en clases **9, 25, 35, 38 y 42**. ([por qué](casillas.md#decision-de-marca-tomada-e-implantada-el-26))
- [x] HACER PÚBLICO EL REPO. **Hecho, y el plan no se había enterado.** Comprobado el 03-09-2026 contra la API (`gh api repos/marcosmatalab/plazum --jq .private` devuelve `false`). ([por qué](casillas.md#hacer-publico-el-repo-hecho-y-el-plan))
...
- [x] Reactivar el workflow codeql. **Reactivado** en el commit 85c5975 («lo que dormía era el nombre del fichero») y corriendo: el análisis de main del 03-09-2026 subió su SARIF y el code scanning tiene análisis con resultados. ([por qué](casillas.md#reactivar-el-workflow-codeql-reactivado-en-el-commit))
...
- [x] `plazum verify` funciona out of the box sobre el demo. El demo se sella UNA VEZ contra una TSA real (`herramientas/sellardemo`, que sale a la red a mano y nunca en CI) y el binario trae embebidas las raíces de FreeTSA y Certum, que son certificados públicos y redistribuibles: un verificador que no trae raíces no es usable offline, y offline es su razón de existir. ([por qué](casillas.md#plazum-verify-funciona-out-of-the-box-sobre))
- [x] Fuzzing nativo de Go del linter de corpus y del verificador comprometido (semillas corren en cada go test)
- [x] Workflow de release: **6 destinos** (3 sistemas × amd64 y arm64; la casilla decía «4 plataformas» y la matriz da seis, corregido en el barrido del 04-09-2026), SHA256SUMS, SBOM CycloneDX, firma keyless cosign (.github/workflows/release.yml). ([por qué](casillas.md#workflow-de-release-6-destinos-3-sistemas-amd64))
- [ ] HITO: decidir la numeración de la siguiente versión. **El bloqueo de marca murió y el trabajo técnico no queda ninguno**: hay tres releases firmadas y ancladas en Rekor (`v0.1.0-rc1`, `v0.1.0` y `v0.1.1`), así que la identidad lleva publicada semanas. Lo que no existe es la `v0.2`, ni en local ni en `origin`. ([por qué](casillas.md#hito-la-primera-release-firmada))
...
- [x] Puertos de la etapa 2 definidos y congelados, con godoc y suites de contrato. ([por qué](casillas.md#puertos-de-la-etapa-2-definidos-y-congelados))
...
- [x] serve con html/template + htmx vendorizado, go:embed, sesiones, y la orden `plazum serve` que lo levanta. ([por qué](casillas.md#serve-con-html-template-htmx-vendorizado-go-embed))
- [x] Seguridad web como puerta (`superficies/serve`). CSRF exigido por MÉTODO, no por lista: la puerta pregunta al enrutador cuáles son sus rutas y falla si alguna mutante se atiende sin token, con control negativo sobre la cadena desnuda. ([por qué](casillas.md#seguridad-web-como-puerta-superficies-serve-csrf-exigido))
...
- [x] El latido: pulso diario opt-in a plazum.dev/latido (dominio provisional) + aviso si calla 24h + smoke test del canal + estado del planificador en Hoy. ([por qué](casillas.md#el-latido-pulso-diario-opt-in-a-plazum))
...
- [x] Export del log de auditoría a SIEM (JSON líneas) (`superficies/export`, `plazum export`). ([por qué](casillas.md#export-del-log-de-auditoria-a-siem-json))
- [x] i18n es/en con mecanismo de catálogo (`adaptadores/catalogo`), consumido ya por la interfaz. ([por qué](casillas.md#i18n-es-en-con-mecanismo-de-catalogo-adaptadores))
- [x] Litestream documentado + restore drill en CI (base + keystore + blobs, verifica cadena y lápidas). ([por qué](casillas.md#litestream-documentado-restore-drill-en-ci-base-keystore))
- [x] Matrix build Linux/macOS/Windows-Docker + imagen Docker publicada; descargo "no es asesoramiento jurídico" en pie y explain. **CERRADA el 22-09-2026**: la imagen está en `ghcr.io`, es pública y se baja sin credenciales, ejercido con la sesión cerrada y la caché borrada. ([por qué](casillas.md#matrix-build-e-imagen-docker-publicada))
- [x] Vendorizar pkcs7 en adaptadores/tsa/internal/pkcs7 con fuzzing propio. ([por qué](casillas.md#vendorizar-pkcs7-en-adaptadores-tsa-internal-pkcs7-con))
- [x] TTFV sintético en CI <15 min; axe-core cero violaciones; presupuestos (binario <25 MB, arranque <3 s, RAM <256 MB). ([por qué](casillas.md#ttfv-sintetico-en-ci-15-min-axe-core))
...
- [x] Los 30 marcos montados como paquetes con su estratificación legal correcta y linter en CI (paquetes/CORPUS.md): ens con art. 31 bienal + INES anual, rgpd con art. 33 (72 h), cra con art. 14.1 (24 h, vigente 11-09-2026); 12 dorados ejecutándose contra el motor. ([por qué](casillas.md#los-30-marcos-montados-como-paquetes-con-su))
...
**Y esto queda escrito como mérito del censo, porque para esto servía:** midiendo el corpus se encontró un **hueco del motor**. El censo ([`docs/censo-relojes.md`](https://github.com/marcosmatalab/plazum/blob/a1ef407850fba1887977cc255e7eddfd9173d962/docs/censo-relojes.md)) no era contenido, era la medición que decide el orden de todo lo que queda, y lo primero que ha decidido es que el corpus no empieza todavía.

- [x] **Primitiva: el máximo de dos duraciones** ([por qué](casillas.md#primitiva-el-maximo-de-dos-duraciones))
- [x] **Primitiva: el preaviso contractual, que se calcula AL REVÉS** ([por qué](casillas.md#primitiva-el-preaviso-contractual-que-se-calcula-al))
- [x] **Comprobado antes de escribir corpus: qué necesita la familia A del motor** ([por qué](casillas.md#comprobado-antes-de-escribir-corpus-que-necesita-la))
...
- [x] **Paquete `nis1-es` (RD 43/2021), la tabla 3 del anexo transcrita con sus relojes.** ([por qué](casillas.md#paquete-nis1-es-rd-43-2021-la-tabla))
- [x] **Paquete `ai-act` (Reglamento (UE) 2024/1689): el art. 50 primero, y el art. 73 con su vigencia en discusión.** ([por qué](casillas.md#paquete-ai-act-reglamento-ue-2024-1689-el))
- [x] **Paquete `iso42001` referencial (ISO/IEC 42001:2023), misma forma y coste marginal que `iso27001`.** ([por qué](casillas.md#paquete-iso42001-referencial-iso-iec-42001-2023-misma))
- [ ] **Equivalencias `iso42001`↔`ai-act` en formato propio.** ([por qué](casillas.md#equivalencias-iso42001-ai-act-en-formato-propio))
- [x] **La familia A del censo, cerrada de una tacada** ([por qué](casillas.md#la-familia-a-del-censo-cerrada-de-una))
- [x] **`plazum calendario`: los relojes existen y ahora se ven** ([por qué](casillas.md#plazum-calendario-los-relojes-existen-y-ahora-se))
- [ ] **Familia B del censo: la periódica anual, ocho marcos.** ([por qué](casillas.md#familia-b-del-censo-la-periodica-anual-ocho))
...
- [ ] **`nis2-es` preparado como release de vigilancia, ANTES de que exista la ley.** ([por qué](casillas.md#nis2-es-preparado-como-release-de-vigilancia-antes))
- [ ] **Marcar `observable` y declarar su prueba en los 12 marcos de la v1.** ([por qué](casillas.md#marcar-observable-y-declarar-su-prueba-en-los))
...
- [x] **UAR con snapshot firmado** ([por qué](casillas.md#uar-con-snapshot-firmado))
- [x] **Auditoría interna 9.2 con arrastre entre ciclos** ([por qué](casillas.md#auditoria-interna-9-2-con-arrastre-entre-ciclos))
- [x] **Acta 9.3 autogenerada + board pack (LA demo)**. ([por qué](casillas.md#acta-9-3-autogenerada-board-pack-la-demo))
...
- [x] **El camino guiado, de punta a punta y sin salir de `plazum serve`** ([por qué](casillas.md#el-camino-guiado-de-punta-a-punta-y))
...
- [x] **PUERTA D11-a, cero formaciones.** ([por qué](casillas.md#puerta-d11-a-cero-formaciones))
- [x] **PUERTA D11-b, todo estado vacío trae su siguiente paso, con test.** ([por qué](casillas.md#puerta-d11-b-todo-estado-vacio-trae-su))
- [x] **PUERTA D11-c, cada número clicable hasta su derivación.** ([por qué](casillas.md#puerta-d11-c-cada-numero-clicable-hasta-su))
...
- [x] **PUERTA D11-e, TTFV por debajo de 15 minutos medido sobre el camino COMPLETO.** ([por qué](casillas.md#puerta-d11-e-ttfv-por-debajo-de-15))
- [x] **Las familias de guardas del núcleo alcanzan a las pantallas.** ([por qué](casillas.md#las-familias-de-guardas-del-nucleo-alcanzan))
...
- [x] **Búsqueda BM25 sobre el corpus y sobre lo que sube el cliente.** Los embeddings salen de la casilla por D-24: un índice que necesita un modelo para construirse rompe que la búsqueda funcione con `PLAZUM_SIN_IA=1` ([por qué](casillas.md#busqueda-fts5-bm25))
- [x] **Verificador de citas por hash**, determinista, corriendo en cada PR con sus adversariales. **Adelantado de E5** ([por qué](casillas.md#verificador-de-citas-por-hash-determinista-corriendo-en))
...
- [x] **Pieza 2, la pregunta con su consecuencia al lado** ([por qué](casillas.md#pieza-2-la-pregunta-con-su-consecuencia-al))
- [x] **Pieza 3, mapeo de la evidencia que ya tiene**: qué documento suyo satisface qué obligación, con cita ([por qué](casillas.md#pieza-3-mapeo-de-la-evidencia-que-ya))
- [x] **Pieza 4, plan de los primeros 30 días** en la pantalla Hoy: 130 obligaciones en rojo es un muro, y la IA agrupa por trabajo (*«estas catorce se cierran con una sola auditoría»*) ([por qué](casillas.md#pieza-4-plan-de-los-primeros-30))
- [x] **Pieza 7, extracción de metadatos de la evidencia**: sube un PDF y se proponen fecha, alcance, firmante y caducidad ([por qué](casillas.md#pieza-7-extraccion-de-metadatos-de-la))
- [x] **PUERTA: el camino completo en verde con `PLAZUM_SIN_IA=1`.** ([por qué](casillas.md#puerta-el-camino-completo-en-verde-con-plazum))
- [x] **PUERTA: evals adversariales del subconjunto, con la inyección vía documento incluida.** ([por qué](casillas.md#puerta-evals-adversariales-del-subconjunto-con-la-inyeccion))
- [x] **PUERTA: ni una pestaña de chat.** ([por qué](casillas.md#puerta-ni-una-pestana-de-chat))
...
- [x] **A1. Bloque `pruebas` en el esquema de paquete, con su linter** ([por qué](casillas.md#a1-bloque-pruebas-en-el-esquema-de-paquete))
- [x] **A2. `estado.Calcular` cableado a la pantalla de controles** ([por qué](casillas.md#a2-estado-calcular-cableado-a-la-pantalla-de))
- [x] **A3. `adaptadores/recoleccion/manual`** ([por qué](casillas.md#a3-adaptadores-recoleccion-manual))
- [x] **A4. PUERTA de extremo a extremo: una observación metida a mano llega hasta `verify` offline y sale verificable.** ([por qué](casillas.md#a4-puerta-de-extremo-a-extremo-una-observacion))
- [x] **A5. `ErrorRecol` como la tercera forma de la nada aplicada a la recolección** ([por qué](casillas.md#a5-errorrecol-como-la-tercera-forma-de-la))
- [x] **A6. Los 12 paquetes vacíos fuera de `paquetes/`.** ([por qué](casillas.md#a6-los-12-paquetes-vacios-fuera-de-paquetes))
...
- [ ] HITO: **v1 publicada** con los 12 marcos, en español e inglés, con el bloque IA de adopción dentro **y con la demo alojada en pie**. ([por qué](casillas.md#hito-v1-publicada-con-los-12-marcos-en))
...
- [ ] **Delegados: Prowler, OpenSCAP, Trivy, ScubaGear vía OCSF.** ([por qué](casillas.md#delegados-prowler-openscap-trivy-scubagear-via-ocsf))
...
- [ ] **ACELERADOR, ya no puerta (D-20)** ([por qué](casillas.md#acelerador-ya-no-puerta-d-20))
