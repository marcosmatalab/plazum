...
func etiquetaDeExclusion(t *testing.T, fichero string) string {
	t.Helper()
	b, err := os.ReadFile(fichero) // #nosec G304 -- ruta que sale de os.ReadDir(dirDeLasComprobaciones)
	if err != nil {
...
func TestTodoTestFueraDeLaSuiteBloqueanteLoCorreUnWorkflowProgramado(t *testing.T) {
	entradas, err := os.ReadDir(dirDeLasComprobaciones)
	if err != nil {
		t.Fatalf("no puedo leer %s: %v", dirDeLasComprobaciones, err)
	}
	...
	for _, e := range entradas {
		...
		casa, err := build.Default.MatchFile(dirDeLasComprobaciones, e.Name())
		if err != nil {
		...
		fuera[e.Name()] = etiquetaDeExclusion(t, filepath.Join(dirDeLasComprobaciones, e.Name()))
	}

	if dentro == 0 {
		t.Fatal("ni un solo fichero de test de comprobaciones/ entra en el build por defecto: " +
	...
	if len(fuera) != FicherosFueraDeLaSuiteBloqueante {
		t.Errorf("hay %d ficheros de test de comprobaciones/ fuera del build por defecto (%v) y "+
	...
	for etiqueta, quienes := range vets {
		...
		if !usada {
			...
			t.Errorf("%v hacen `go vet -tags %s` y ningun fichero de comprobaciones/ sale del build "+
	...
	for etiqueta, quienes := range porEtiqueta {
		...
		if !usada {
			t.Errorf("%v corren una puerta con `-tags %s` y ningun fichero de comprobaciones/ sale "+
	...
	t.Logf("MEDIDO: %d ficheros de test de comprobaciones/ dentro del build por defecto, %d fuera "+
