...
func casosDeTestEscritos(t *testing.T) int {
	...
	for _, f := range ficherosVersionados(t, "*_test.go") {
		...
		for _, m := range re.FindAll(b, -1) {
			// TestMain no es un caso de test: es el arranque del paquete, y
			// contarlo inflaria la cifra publicada en uno por cada paquete que
			// lo tenga, que es un error a favor.
			if string(m) != "func TestMain" {
				n++
			}
		}
	}
