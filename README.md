...
<!-- binario:inicio -->
**Y lo que ocupa, que casi nadie publica.** El binario de Linux, con `-s -w -trimpath`, mide **12,0 MB** contra un presupuesto declarado de **25 MB**. Se construye así:
...
**Ha subido dos veces y las dos se dicen por qué**, porque un binario que engorda sin explicación es lo que hace que nadie se crea el resto. La primera, **0,4 MB el 03-09-2026**: la release **lleva el corpus dentro**, y eso es lo que convierte una máquina recién instalada de 3 relojes en 263, sin red y sin pasos extra. La segunda, **168 KiB el 10-09-2026**, al entrar la lectura de los documentos que subes: extracción de texto, índice de búsqueda y verificación de citas por hash, que es lo que hace que plazum te enseñe el párrafo de tu propia política al lado de la norma que lo pide. El presupuesto no se ha movido para acomodar ninguna de las dos.

Las dos cifras salen del **mismo banco**, que es lo que permite restarlas: `go1.24.13` sobre `linux/amd64` nativo, con las banderas de arriba. Dos medidas de máquinas distintas no se restan nunca, y aquí también se aprendió en rojo.
<!-- binario:fin -->
